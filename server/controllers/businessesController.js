...
businessesController.getBusinesses = (req, res, next) => {
  ...
  console.log('from getBusiness', category, lat, lon);
  // log error if latitude or longitude are undefined
  if (lat === undefined || lon === undefined) {
    return next({
      log:
        'businessController.getBusinesses: ERROR: lat and/or lon are undefined',
      message: {
        err:
          'businessesController.getBusinesses: ERROR: Check server logs for details',
      },
    });
  }
...
businessesController.getBusinessesByPrice = (req, res, next) => {
  ...
  if (priceLevel < 1 || priceLevel > 4) {
    return next({
      log:
        'businessesController.getBusinessesByPrice: ERROR: priceLevel is invalid. priceLevel must be: 1 <= priceLevel <= 4.',
      message: {
        err:
          'businessesController.getBusinessesByPrice: ERROR: Check server logs for details',
      },
    });
  }
